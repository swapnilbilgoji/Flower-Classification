...
# Config / model path
# ---------------------------
MODEL_PATH = "Fruit_Classification_Model.h5"   # local filename to use (or change to your .h5)
...
# ---------------------------
# Google Drive download helpers (same as before)
# ---------------------------
...
# ---------------------------
# Helpers (image/predict/plots) - uses rescale 1/255 (detected from your notebook)
# ---------------------------
def preprocess_pil_exact_rescale(img: Image.Image, target_size=(224,224)):
    ...
    arr = np.asarray(img).astype(np.float32) / 255.0   # <-- MATCHES training: rescale 1./255
...
def load_model_with_fallback(local_path, image_size=(224,224), num_classes=3, debug=False):
    ...
    # try local full model load first
    ...
    # if missing, download from Drive
    ...
    # try full load again after download
    try:
        model = tf.keras.models.load_model(local_path)
        try:
            shape = model.input_shape
    ...
    # fallback: build single-input model and load weights by_name
    try:
...
# ---------------------------
# Load model metadata (if present) to get target size & class mapping
# ---------------------------
if os.path.exists(META_PATH):
    try:
        with open(META_PATH, "r") as f:
            meta = json.load(f)
        TARGET_SIZE_META = tuple(meta.get("target_size", (224,224)))
        PREPROCESSING = meta.get("preprocessing", "rescale_1/255")
        CLASS_INDICES = meta.get("class_indices")
    except Exception:
        TARGET_SIZE_META = (224,224)
        PREPROCESSING = "rescale_1/255"
        CLASS_INDICES = None
else:
    TARGET_SIZE_META = (224,224)
    PREPROCESSING = "rescale_1/255"
...
# invert class indices if available (index->label)
if CLASS_INDICES:
    IDX_TO_CLASS = {int(v): k for k, v in CLASS_INDICES.items()}
else:
    IDX_TO_CLASS = None  # fallback: use RIPENESS_CLASSES
...
# ---------------------------
try:
    model, TARGET_SIZE = load_model_with_fallback(MODEL_PATH, image_size=TARGET_SIZE_META, num_classes=3, debug=False)
except Exception as e:
    st.sidebar.error(f"Failed to load model at '{MODEL_PATH}': {e}")
    st.sidebar.markdown(
        """
        **How to fix**:
        1. Upload your model to Google Drive and make it shareable.  
        2. Put the file id into Streamlit secrets as `GDRIVE_FILE_ID` or the full shareable link as `GDRIVE_SHAREABLE_LINK`.  
        3. Ensure model_metadata.json is present and contains `"target_size": [224,224]` and `"preprocessing": "rescale_1/255"`.  
        """
    )
    st.stop()
...
with col1:
    ...
    if img_pil is not None:
        with st.spinner("Running model inference..."):
            preds = predict_with_preproc(img_pil, model, target_size=TARGET_SIZE_META)
        ...
        conf_table = { (IDX_TO_CLASS[i] if IDX_TO_CLASS else RIPENESS_CLASSES[i]): float(probs[i]) for i in range(len(probs)) }
        st.json(conf_table)
